...
interface SelectorProps {
	name: string;
	onChange: (selectedItems: SpeakerEmployee[]) => void;
	items: SpeakerEmployee[];
}

export const SelectorEmployee: React.FC<SelectorProps> = ({
	name,
	onChange,
	items,
}) => {
	const [selectedItems, setSelectedItems] = useState<SpeakerEmployee[]>([]);
	...
	const handleSelectItem = (item: SpeakerEmployee) => {
		const isSelected = selectedItems.some(
			(selected) => selected.identificacion === item.identificacion
		);
		if (isSelected) {
			setSelectedItems((prevItems) =>
				prevItems.filter(
					(selected) =>
						selected.identificacion !== item.identificacion
				)
			);
		} else {
